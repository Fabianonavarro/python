import tkinter as tk
from tkinter import messagebox, filedialog
import requests
import csv
from datetime import date

class CepApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Consulta CEP")
        
        # Definindo a dimensão da janela e desabilitando o botão de maximizar
        self.root.geometry("500x400")
        self.root.resizable(False, False)  # Desabilita o redimensionamento da janela
        
        # Configura a geometria mínima da janela
        self.root.minsize(500, 400)
        
        self.create_widgets()
        self.update_date()
    
    def create_widgets(self):
        # Configura o label da data
        self.date_label = tk.Label(self.root, text="", font=("Arial", 12))
        self.date_label.pack(pady=10)
        
        # Configura o label e a entrada de CEP
        self.cep_label = tk.Label(self.root, text="Digite o CEP:", font=("Arial", 12))
        self.cep_label.pack()
        
        self.cep_entry = tk.Entry(self.root, font=("Arial", 12))
        self.cep_entry.pack(pady=5)
        
        # Configura o frame para os botões de consulta, nova consulta, copiar e exportar
        button_frame = tk.Frame(self.root)
        button_frame.pack(pady=10)
        
        # Botão Consultar
        self.search_button = tk.Button(button_frame, text="Consultar", font=("Arial", 12), width=15, command=self.consultar_cep)
        self.search_button.grid(row=0, column=0, padx=5)
        
        # Botão Nova Consulta
        self.new_search_button = tk.Button(button_frame, text="Nova Consulta", font=("Arial", 12), width=15, command=self.clear)
        self.new_search_button.grid(row=0, column=1, padx=5)
        
        # Botão Copiar
        self.copy_button = tk.Button(button_frame, text="Copiar", font=("Arial", 12), width=15, command=self.copy_to_clipboard)
        self.copy_button.grid(row=1, column=0, padx=5, pady=5)
        
        # Botão Exportar
        self.export_button = tk.Button(button_frame, text="Exportar", font=("Arial", 12), width=15, command=self.export_to_csv)
        self.export_button.grid(row=1, column=1, padx=5, pady=5)
        
        # Botão Sair
        self.exit_button_frame = tk.Frame(self.root)
        self.exit_button_frame.pack(pady=10)
        
        self.exit_button = tk.Button(self.exit_button_frame, text="Sair", font=("Arial", 12), width=15, command=self.quit_app, bg="red", fg="white")
        self.exit_button.pack()
        
        # Indicador de carregamento
        self.loading_label = tk.Label(self.root, text="", font=("Arial", 12), fg="blue")
        self.loading_label.pack(pady=5)
        
        # Configura a área de texto para exibir resultados com cor de fundo azul claro
        self.result_text = tk.Text(self.root, height=8, width=60, font=("Arial", 12), wrap=tk.WORD, bg="#e0f7fa")
        self.result_text.pack(pady=10)
    
    def update_date(self):
        current_date = date.today()
        formatted_date = current_date.strftime('%d/%m/%Y')
        self.date_label.config(text=f"Data: {formatted_date}")
    
    def consultar_cep(self):
        cep_input = self.cep_entry.get().strip()
        
        if len(cep_input) != 8 or not cep_input.isdigit():
            messagebox.showerror("Erro", "Quantidade de dígitos inválida ou o CEP contém caracteres não numéricos!")
            return
        
        # Mostrar indicador de carregamento
        self.loading_label.config(text="Carregando...")
        self.root.update_idletasks()  # Atualizar a interface para mostrar a mensagem imediatamente
        
        try:
            response = requests.get(f'https://viacep.com.br/ws/{cep_input}/json/')
            busca_cep = response.json()
            
            if "erro" not in busca_cep:
                result = (f"CEP: {busca_cep['cep']}\n"
                          f"Endereco: {busca_cep['logradouro']}\n"
                          f"Bairro: {busca_cep['bairro']}\n"
                          f"Cidade: {busca_cep['localidade']}\n"
                          f"Estado: {busca_cep['uf']}")
                self.result_text.delete(1.0, tk.END)
                self.result_text.insert(tk.END, result)
                self.loading_label.config(text="")  # Limpar indicador de carregamento
            else:
                messagebox.showerror("Erro", f"Este CEP {cep_input} é inválido. Favor verificar.")
                self.loading_label.config(text="")  # Limpar indicador de carregamento
        
        except Exception as e:
            messagebox.showerror("Erro", f"Erro ao consultar o CEP: {e}")
            self.loading_label.config(text="")  # Limpar indicador de carregamento
    
    def copy_to_clipboard(self):
        result = self.result_text.get(1.0, tk.END).strip()
        if result:
            self.root.clipboard_clear()
            self.root.clipboard_append(result)
            messagebox.showinfo("Copiar", "Endereço copiado para a área de transferência!")
        else:
            messagebox.showwarning("Aviso", "Nenhum resultado para copiar.")
    
    def export_to_csv(self):
        result = self.result_text.get(1.0, tk.END).strip()
        if result:
            # Solicitar ao usuário o nome do arquivo CSV
            file_path = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV files", "*.csv")])
            if file_path:
                try:
                    # Escrever os dados no arquivo CSV
                    with open(file_path, mode='w', newline='', encoding='utf-8') as file:
                        writer = csv.writer(file)
                        writer.writerow(["CEP", "Endereco", "Bairro", "Cidade", "Estado"])
                        lines = result.split("\n")
                        # Escreve cada linha separada no arquivo CSV
                        writer.writerow(lines)
                    messagebox.showinfo("Exportar", "Dados exportados com sucesso!")
                except Exception as e:
                    messagebox.showerror("Erro", f"Erro ao exportar para CSV: {e}")
        else:
            messagebox.showwarning("Aviso", "Nenhum resultado para exportar.")
    
    def clear(self):
        self.cep_entry.delete(0, tk.END)
        self.result_text.delete(1.0, tk.END)
        self.loading_label.config(text="")  # Limpar indicador de carregamento
    
    def quit_app(self):
        if messagebox.askyesno("Sair", "Você realmente deseja sair?"):
            self.root.destroy()  # Corrige a funcionalidade para fechar a janela

if __name__ == "__main__":
    root = tk.Tk()
    app = CepApp(root)
    root.mainloop()
